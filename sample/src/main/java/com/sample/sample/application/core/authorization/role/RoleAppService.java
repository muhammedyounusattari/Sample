...
import com.sample.sample.domain.core.authorization.role.QRoleEntity;
...
public class RoleAppService implements IRoleAppService {
    ...
    public UpdateRoleOutput update(Long roleId, UpdateRoleInput input) {
        ...
        RoleEntity updatedRole = _roleRepository.save(role);
        return mapper.roleEntityToUpdateRoleOutput(updatedRole);
    }

    @Transactional(propagation = Propagation.REQUIRED)
    public void delete(Long roleId) {
        RoleEntity existing = _roleRepository.findById(roleId).orElse(null);
    ...
    public FindRoleByIdOutput findById(Long roleId) {
        RoleEntity foundRole = _roleRepository.findById(roleId).orElse(null);
    ...
    public FindRoleByNameOutput findByRoleName(String roleName) {
        RoleEntity foundRole = _roleRepository.findByRoleName(roleName);
        if (foundRole == null) {
            return null;
        }
    ...
    public List<FindRoleByIdOutput> find(SearchCriteria search, Pageable pageable) throws Exception {
        Page<RoleEntity> foundRole = _roleRepository.findAll(search(search), pageable);
